...
class TestImage:
    ...
    def test_hypothesis_invalid_string_input(self, invalid_str):
        ...
        if not Image._Image__check_if_base64(invalid_str) and not invalid_str.startswith(('http', 'file')):
    ...
    def test_create_from_url(self):
        image = Image(image="https://raw.githubusercontent.com/tapis-project/camera-traps/refs/heads/main/installer/example_images/baby-red-fox.jpg")
        assert isinstance(image.image, PILImage.Image)
    ...
    @given(arrays(dtype=np.uint8, shape=st.tuples(
        st.integers(1, 10), st.integers(1, 10), st.integers(5, 10))))
    ...
    @given(st.one_of(
        arrays(dtype=np.complex64, shape=st.tuples(
            st.integers(1, 10), st.integers(1, 10), st.integers(1, 3))),
        arrays(dtype=np.bool_, shape=st.tuples(
            st.integers(1, 10), st.integers(1, 10), st.integers(1, 3))),
    ))
